...
export default function HabitItem({ habit, onToggle, onDelete }) {
  ...
  return (
    <div className={`
      group relative rounded-2xl backdrop-blur-sm border transition-all duration-500
      bg-gradient-to-br ${variant.gradient} ${variant.border}
      hover:shadow-2xl hover:shadow-slate-900/40 hover:-translate-y-1
      ${habit.completed ? 'opacity-95 hover:opacity-100' : ''}
      ${isDeleting ? 'scale-95 opacity-50' : ''}
    `}>
      {/* Animated accent bar */}
      <div className={`absolute top-0 left-0 right-0 h-1 ${variant.accent} opacity-90 group-hover:opacity-100 transition-all duration-300`} />
      
      {/* Enhanced glow effect */}
      <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${variant.glow} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl -z-10`} />
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-6 mb-6">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 mb-3">
              {habit.completed && (
                <div className="flex items-center justify-center w-8 h-8 bg-green-500/25 rounded-full border border-green-500/40 shadow-lg shadow-green-500/20 transition-all duration-300 group-hover:scale-110">
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`
                  text-xl font-semibold transition-all duration-300 truncate
                  ${habit.completed 
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {habit.priority && !habit.completed && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border backdrop-blur-sm bg-black/25 transition-all duration-300 hover:scale-105">
                  <div className={`w-2 h-2 rounded-full ${variant.accent} shadow-sm`} />
              ...
              {habit.category && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-slate-300 border border-slate-600/40 bg-black/25 backdrop-blur-sm transition-all duration-300 hover:scale-105">
                  <svg className="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                  </svg>
                  <span className="text-xs font-medium">{habit.category}</span>
                </div>
              )}

              {habit.streak > 0 && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-orange-300 border border-orange-500/40 bg-orange-500/15 backdrop-blur-sm transition-all duration-300 hover:scale-105">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M12.395 2.553a1 1 0 00-1.45-.385c-.345.23-.614.558-.822.88-.214.33-.403.713-.57 1.116-.334.804-.614 1.768-.84 2.734a31.365 31.365 0 00-.613 3.58 2.64 2.64 0 01-.945-1.067c-.328-.68-.398-1.534-.398-2.654A1 1 0 005.05 6.05 6.981 6.981 0 003 11a7 7 0 1011.95-4.95c-.592-.591-.98-.985-1.348-1.467-.363-.476-.724-1.063-1.207-2.03zM12.12 15.12A3 3 0 017 13s.879.5 2.5.5c0-1 .5-4 1.25-4.5.5 1 .786 1.293 1.371 1.879A2.99 2.99 0 0113 13a2.99 2.99 0 01-.879 2.121z"/>
                  </svg>
                  <span className="text-xs font-semibold">{habit.streak} day streak</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 shrink-0">
            <button
              ...
              className={`
                ...
                backdrop-blur-sm border hover:shadow-lg active:scale-95 min-w-[120px]
            ...
            <button
              ...
              disabled={isDeleting}
              className="
                px-5 py-2.5 rounded-xl text-sm font-semibold text-slate-300 
                ...
                active:scale-95 min-w-[120px] flex items-center justify-center gap-2
        ...
        <div className="space-y-4 pt-5 border-t border-slate-700/40">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold text-blue-300">{progress}%</span>
              {progress === 100 && (
          ...
          <div className="w-full h-3 bg-slate-700/60 rounded-full overflow-hidden border border-slate-600/40 backdrop-blur-sm">
            <div
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <div className="flex items-center gap-2">
              <span className="text-slate-400">Milestone: {progress >= 50 ? '50%' : '25%'}</span>
              <div className="w-1 h-1 bg-slate-600 rounded-full" />
              <span className="text-slate-300 font-semibold">Goal: 100%</span>
            </div>
          </div>
        </div>
      </div>
